...
/*!
  @brief   NeoPixel constructor when length, are known
           at compile-time. for WLED output
  @param   n  Number of NeoPixels in strand.
  @param   t  Pixel type -- add together NEO_* constants defined in
              Adafruit_NeoPixel.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
  ...
  @return  Adafruit_NeoPixel object. Call the begin() function before use.
*/
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, IPAddress wledip, neoPixelType t)
    : begun(false), brightness(0), pixels(NULL), endTime(0)
{
    updateType(t);
    updateWLED(true);
...
void Adafruit_NeoPixel::updateLength(uint16_t n)
{
    ...
    if ((pixels = (uint8_t *)malloc(numBytes)))
    {
        ...
        lastpacketcount = (numBytes % split);

        packetloop = packetcount;
...
void Adafruit_NeoPixel::updateWLED(bool t)
{
    wled = t;
}
...
void Adafruit_NeoPixel::setIPAddress(IPAddress)
{
}
